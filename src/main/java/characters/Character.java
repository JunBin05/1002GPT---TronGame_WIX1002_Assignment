...
public abstract class Character {
    ...
    // Pending turn requested by player input.
    protected char pendingDirection = '\0';

    public void requestDirection(char dir) {
        this.pendingDirection = java.lang.Character.toUpperCase(dir);
    }
    ...
    public void prepareForNextStage() {
        ...
        System.out
                .println(name + " Ready for Stage. Discs: " + currentDiscCount + "/" + discCapacity + " HP: " + lives);
        if (this.isPlayer) {
            arena.ArenaLoader.appendGameplayLog(
                    "Ready for Stage. Discs: " + currentDiscCount + "/" + discCapacity + " HP: " + lives);
        }
    }
    ...
    public void setXp(long xp) {
        if (xp < 0)
            xp = 0;
        this.currentXp = xp;
        // Recompute level to match XP (start from current level, levelUp will apply
        // buffs)
    ...
    public String commitPendingXP(String username) {
        ...
        if (username != null && !username.trim().isEmpty()) {
            try {
                ...
                if (this.name.equals("Tron"))
                    savedMaxLevel = db.getTronLevel(username);
                else if (this.name.equals("Kevin"))
                    savedMaxLevel = db.getKevinLevel(username);
            } catch (Exception e) {
        ...
        // We intentionally avoid simulating base attributes from CharacterData here to
        // keep the UI
        // method lightweight and reduce failure surface; saved max-level (from DB) is
        // still respected via `displayOldLevel`/`displayNewLevel` computed earlier.
        ...
        sb.append(
                "<html><body style='width: 300px; background-color: #222; color: #00FFCC; font-family: Sans-serif; padding: 10px;'>");
        ...
        sb.append(String.format("<tr><td>XP Gained:</td><td style='text-align: right; color: #00FF00;'>+%d</td></tr>",
                gained));
        ...
        if (didLevelUp || displayOldLevel != displayNewLevel) {
            ...
            sb.append(String.format(
                    "<tr><td><b>LEVEL:</b></td><td style='text-align: right; color: yellow;'><b>%d &rarr; %d</b></td></tr>",
                    displayOldLevel, displayNewLevel));
            sb.append(String.format(
                    "<tr><td>Speed:</td><td style='text-align: right;'>%.2f &rarr; <span style='color: #00FFCC;'>%.2f</span></td></tr>",
                    displayOldSpeed, displayNewSpeed));
            sb.append(String.format(
                    "<tr><td>Handling:</td><td style='text-align: right;'>%.2f &rarr; <span style='color: #00FFCC;'>%.2f</span></td></tr>",
                    displayOldHandling, displayNewHandling));

            // Show Capacity Increase (show new capacity)
            sb.append(String.format("<tr><td>Disc Cap:</td><td style='text-align: right;'>%d</td></tr>",
                    displayNewDiscCap));
            sb.append(String.format("<tr><td>Max Lives:</td><td style='text-align: right;'>%.1f</td></tr>",
                    displayNewMaxLives));
        } else {
            sb.append("<tr><td colspan='2'><br><i style='color: #888;'>Progress to Level " + (displayNewLevel + 1)
                    + "...</i></td></tr>");
        }
    ...
    public void levelUp() {
        ...
        // Persist XP immediately when leveling up for logged-in users so UI remains
        // consistent
        try {
                ...
                if (user != null && !user.trim().isEmpty()) {
                    new Thread(() -> {
                        try {
                            UI.DatabaseManager db = new UI.DatabaseManager();
                            if (this.name.equals("Tron"))
                                db.setTronXp(user, this.currentXp);
                            else if (this.name.equals("Kevin"))
                                db.setKevinXp(user, this.currentXp);
                        } catch (Exception e) {
    ...
    public void throwDisc() {
        if (currentDiscCount > 0) {
            ...
            if (this.isPlayer) {
                arena.ArenaLoader
                        .appendGameplayLog("Used a disc (Discs: " + currentDiscCount + "/" + discCapacity + ")");
            }
        }
    }
    ...
    public void pickupDisc() {
        if (currentDiscCount < discCapacity) {
            ...
            if (this.isPlayer) {
                arena.ArenaLoader
                        .appendGameplayLog("Picked up a disc (Discs: " + currentDiscCount + "/" + discCapacity + ")");
            }
        }
    }
    ...
    public void setDiscsOwned(int owned) {
        this.discsOwned = Math.max(0, owned);
    }

    public void setDiscCapacity(int cap) {
        this.discCapacity = Math.max(0, cap);
        if (this.currentDiscCount > this.discCapacity)
            this.currentDiscCount = this.discCapacity;
    }
    ...
    public long getNextDiscReadyNs() {
        return this.nextDiscReadyNs;
    }

    public void setNextDiscReadyNs(long ns) {
        this.nextDiscReadyNs = ns;
    }
    ...
    public void changeLives(double amount) {
        ...
        if (amount < 0) {
            ...
            arena.ArenaLoader.appendGameplayLog(this.name + " took " + String.format("%.1f", -amount) + " damage (HP: "
                    + String.format("%.1f", this.lives) + "/" + String.format("%.1f", this.maxLives) + ")");
        }
    ...
    // Public setters so callers (e.g., LevelManager) can tune health values
    // per-stage
    ...
    public void setTrailDuration(int duration) {
        this.trailDuration = Math.max(1, duration);
    }
