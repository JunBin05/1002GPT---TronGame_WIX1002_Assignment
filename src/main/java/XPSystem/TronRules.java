...
public class TronRules {
    ...
    private static double computeProgressionScale() {
        if (!PROGRESSIVE_SCALING_ENABLED)
            return 1.0;
        if (progressionScaleCache != null)
        ...
        if (totalUnscaled <= 0.0) {
            progressionScaleCache = 1.0;
            return progressionScaleCache;
        }
        progressionScaleCache = target / totalUnscaled;
        System.out.println(String.format("[TronRules] Progression scaling: target=%,.0f totalUnscaled=%,.0f scale=%.4f",
                target, totalUnscaled, progressionScaleCache));
        return progressionScaleCache;
    }
    ...
    static {
        ...
        System.out.println(String.format(
                "[TronRules] Fixed XP table initialized. Full-run total=%,d required=%,d, C5S4=%d (replay cap %.0f)",
                total, requiredTotal, FIXED_STAGE_XP[5][4], FIXED_STAGE_XP[5][4] / (1.0 - decay)));
    }

    public static long calculateStageClearXp(int chapter, int stage) {
        if (chapter < 1 || chapter > 5)
            return 0;
        if (stage < 1 || stage > STAGES_PER_CHAPTER[chapter])
            return 0;
    ...
    public static double calculateReplayMultiplier(int playerLevel, int chapter, int stage) {
        ...
        if (mult < STAGE_REPLAY_MIN_FACTOR)
            mult = STAGE_REPLAY_MIN_FACTOR;
        return mult;
    }
}